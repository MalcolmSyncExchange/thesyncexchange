...
## Branded cursor and hero artwork

Replaced the generic microphone with a custom blue-glass/chrome sound-wave sculpture, accented in gold. The production WebP is 119 KB and stored at `public/images/sync-sound-sculpture.webp`. It was generated with the built-in Image Generation tool and converted to WebP without changing composition. Prompt: "Premium music licensing homepage hero; one sculptural flowing audio waveform ribbon of smoked chrome and translucent blue glass, rhythmic fine grooves above a matte near-black surface, subtle gold edges, cyan reflections, charcoal/navy #10171c background, cinematic studio lighting and spacious 6:5 composition. Keep subject central for mobile crop. No microphone, headphones, people, words, logos, letter X, watermark or UI. Photographic 3D still."

Added Lucide-derived cyan arrow/hand cursor assets and a soft blue halo shared across the application. The halo is decorative, ignores pointer input, and updates at most once per animation frame. It hides for keyboard tabbing, pointer leave, window blur and visibility changes. Touch, reduced-motion and forced-color environments use the normal system cursor without the halo. No authentication, data or payment logic changed.

Webpack production build including type checking passed; lint passed with the existing React Hook Form warning. Browser verification passed after retry: artwork loads at desktop and narrow widths, glow intensity is 20% of the initial effect, pointer movement updates its position, keyboard Tab hides it, the overlay ignores clicks, the buyer signup link works and text inputs retain the text cursor. No new unit tests were added for this visual-only change.
