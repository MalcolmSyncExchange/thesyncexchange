...
function loadModule(path, stubs) {
  ...
  runInNewContext(outputText, {
    module, exports: module.exports, Response, Request, FormData, File,
...
function limiter({ rows = [allowedRow], error = null, throws = false, missingClient = false, target = "preview", rpcOverride } = {}) {
  const calls = [];
...
function harness({ role = "buyer", authenticated = true, owner = userId, pricingError = false, limitOptions = {} } = {}) {
  ...
  const stubs = {
    ...
    "next/dist/client/components/redirect": { isRedirectError: e => e.digest === "NEXT_REDIRECT" },
    ...
    "@/services/supabase/server": { createServerSupabaseClient: () => db },
    "@/services/supabase/privileged": { createPrivilegedSupabaseClient: () => db },
